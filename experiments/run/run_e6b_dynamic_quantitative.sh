...
CONFIG="$ROOT/experiments/configs/e6b_dynamic_quantitative.json"
OUT="$ROOT/results/e6b_dynamic_quantitative"
mkdir -p "$OUT"
...
python3 -c 'import json, pathlib, sys; s=json.loads(pathlib.Path("'$OUT'/summary.json").read_text()); s["experiment_name"]="e6b_dynamic_quantitative"; pathlib.Path("'$OUT'/summary.json").write_text(json.dumps(s,indent=2)+"\n"); sys.exit(0 if s.get("simulator")=="ns-3" and s.get("fallback_used") is False else 1)'
