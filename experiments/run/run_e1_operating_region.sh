...
OUT="$ROOT/results/e1_operating_region"
mkdir -p "$OUT"
...
(cd "$NS3_ROOT" && ./ns3 run "ev-plc-transition-admission --config=$CONFIG --output=$OUT --experiment=exp4_three_regime")
python3 -c 'import json, pathlib, sys; s=json.loads(pathlib.Path("'$OUT'/summary.json").read_text()); s["experiment_name"]="e1_operating_region"; pathlib.Path("'$OUT'/summary.json").write_text(json.dumps(s,indent=2)+"\n"); sys.exit(0 if s.get("simulator")=="ns-3" and s.get("fallback_used") is False else 1)'
